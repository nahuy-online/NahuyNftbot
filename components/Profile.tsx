...
export const Profile: React.FC<ProfileProps> = ({ user, onUpdate }) => {
  ...
  const handleWithdraw = async () => {
      ...
      if (user.nftBalance.available <= 0) {
          if (window.Telegram?.WebApp) {
              window.Telegram.WebApp.showAlert("No available NFTs to withdraw.");
          }
          return;
      }
      
      const confirm = window.confirm(`Withdraw ${user.nftBalance.available} NFTs to ${targetAddress.slice(0,4)}...${targetAddress.slice(-4)}?`);
      if(confirm) {
          setWithdrawing(true);
          try {
    ...
    <div className="p-5 pb-24 space-y-6 animate-fade-in">
      ...
      <div className="bg-gray-800/60 backdrop-blur-md rounded-2xl p-4 border border-white/5">
          <div className="flex justify-between items-center mb-3">
              <span className="text-gray-400 text-sm font-medium">Connect Wallet</span>
      ...
      {/* Balance Grid */}
      <div>
        <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3 ml-1">NFT Assets</h3>
        <div className="grid grid-cols-2 gap-3">
            {/* Total */}
            <div className="bg-gray-800 p-4 rounded-2xl border border-white/5 flex flex-col justify-between h-28">
                <div className="text-gray-400 text-xs font-bold uppercase">Total Balance</div>
                <div className="text-3xl font-black text-white">{user.nftBalance.total}</div>
            ...
            <div className="bg-gray-800 p-4 rounded-2xl border border-yellow-500/20 flex flex-col justify-between h-28 relative overflow-hidden">
                ...
                <div className="text-yellow-500/80 text-xs font-bold uppercase z-10">Locked</div>
                <div className="text-3xl font-black text-yellow-500 z-10">{user.nftBalance.locked}</div>
                <div className="text-[10px] text-gray-500 font-medium z-10">Unlocks gradually</div>
            </div>
        </div>
      </div>
      ...
      <div className="glass-panel p-5 rounded-2xl">
          <div className="flex justify-between items-end mb-4">
              <div>
                <span className="text-gray-300 font-medium text-sm block mb-1">Available to Withdraw</span>
          ...
          >
              {withdrawing ? 'Processing on Blockchain...' : 'Withdraw to Wallet'}
          </button>
      </div>
      ...
      {user.nftBalance.lockedDetails && user.nftBalance.lockedDetails.length > 0 && (
          <div className="space-y-2">
              <h4 className="text-xs font-bold text-gray-500 uppercase ml-1">Vesting Schedule</h4>
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
               {['Level 1', 'Level 2', 'Level 3'].map((lvl, i) => (
                   <div key={i} className="bg-gray-800 p-2 rounded-lg text-center border border-white/5">
                       <div className="text-[10px] text-gray-500 uppercase">{lvl}</div>
